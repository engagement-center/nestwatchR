...
staff have identified in order to help analysts clean NestWatch data for
analysis. Please read the full function documentation and this vignette
...
NestWatch additionally implements geographic filters on real-time
entered species, which are curated by [eBird](https://ebird.org/home).
For example, a participant should not be able to report a nest from a
species that is extremely rare in that region, preventing many species
misidentifications. However, for similar sympatric species, incorrect
identifications are still possible (e.g., a Black-capped Chickadee may
be misidentified as a Carolina Chickadee in regions of range overlap).
However, older reports, including historic data migrated to the
NestWatch database for archival, are not subjected to the same rigorous
filters. Additionally, errors in reporting can mimic errors in
identification. Participants may intend to report one species but enter
the data for the wrong species. We recommend that data analysts map nest
locations and screen for likely errors. See the
...
(this typically affects a very low number of records). Previous versions
...
When certainty of a specie(s) is needed, the analyst may decide to omit
spuhs and slashes as species is not known with certainty:
...
Some subspecies or forms are easily differentiated and may be specified
by a participant. Caution should be taken when using these
subspecies/form designations as the identification skills of the
participant are unknown and the participant may not have selected the
...
Documentation of nesting attempts between different parent species, a
hybrid parent, or two hybrid parents are very uncommon. The NestWatch
database has no way to easily differentiate between these three cases.
Thus, if a hybrid species was selected for an attempt, the parent
specie(s) of the nest vs the resulting young specie will not be known
with certainty. Additionally, participants may incorrectly select a
hybrid species when they should have selected a spuh or slash species
designation. If you are interested in validating attempts labeled with
hybrid status, you may [request additional
...
have identified to assist analysts in cleaning NestWatch data at a
coarse scale. You will need to specify which procedures to run, lettered
`a:k` (not case sensitive), and we highly suggest you read the function
documentation and the following examples while considering your analyses
before selecting which procedures to run. After these coarse scale
cleanings are completed, the user should continue cleaning the data at a
finer scale including considering spatial and phenological filtering.
See the [Filtering
Vignette](https://engagement-center.github.io/nestwatchR/articles/c_Data-Filtering.html)
and
[`nw.filterspatial()`](https://engagement-center.github.io/nestwatchR/reference/nw.filterspatial.html)
...
> those species prior to running
> [`nw.cleandata()`](https://engagement-center.github.io/nestwatchR/reference/nw.cleandata.html).
...
b.  **No Nesting Activity:** Flag/remove attempts with Outcome of "no
    ...
    which never received eggs or were unmonitored. Users may choose to
    include these data if looking at habitat or location data without
    measures of phenology or success. <br> <br>
c.  **Invasive Species Management:** Flag/remove attempts with Outcome
    ...
    estimates would not be meaningful. Note: “Invasive species
    management” may also be incorrectly chosen by participants trying to
    indicate nest failure caused by an invasive species, in which case
    it would still be prudent to flag or remove such records. <br> <br>
d.  **Failed but fledged?:** Flag/remove attempts if outcome is a
    ...
    Discerning the true outcome may not be possible or expedient. If an
    attempt produces any number of fledged host young, the attempt is
    considered successful.<br> <br>
...
i.  **Decrease-then-increase Eggs/Young:** Flag/remove attempts where
    ...
    <br>
j.  **Impossibly long attempts:** Flag attempts with impossible nesting
...
k.  **Year errors in visits:** Flag/remove attempts where the \# days
    ...
    review these attempts individually to verify if a typo occurred.<br>
    <br>

Depending on what cleaning procedures you choose to run, the sample size
of nests may decrease to varying degrees. If your species has a small
