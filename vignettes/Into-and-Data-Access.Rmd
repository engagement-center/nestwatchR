...
vignette: >
  ...
  %\VignetteIndexEntry{Introduction to NestWatch Data and Data Access}
...
Our ability to allocate conservation efforts of the natural world depends on detailed information about the life histories, distributions, abundances, and population trends of species over time. The goal of the [NestWatch project](https://nestwatch.org), administered by [The Cornell Lab of Ornithology](https://www.birds.cornell.edu/home/), is to use data collected by members of the global participatory science community and researchers to archive nest location and phenology information for the world’s bird populations. NestWatch is designed to track status and trends in the reproductive biology of birds, including when nesting occurs, number of eggs laid, how many eggs hatch, and how many hatchlings survive. Our database is intended to be used to study the current condition of breeding bird populations and how they may be changing over time as a result of climate change, habitat degradation and loss, expansion of urban areas, and the introduction of non-native plants and animals.

This vignette gives an overview of the NestWatch Data Products, which represent both nest-level parameters and individual nest checks. Knowledge of the structure of the database is critical for proper interpretation of the dataset. This example will discuss some of the 54 data fields, but we ask all users of the data to review the field explanations in the data paper [Bailey et al. 2024](https://doi.org/10.1002/ecy.4230).



# Data Access {#access}

The NestWatch database is freely accessible via a [Mendeley data repository](https://data.mendeley.com/datasets/wjf794z7gc), the [NestWatch website](https://nestwatch.org/explore/nestwatch-open-dataset-downloads/), or the `nestwatchR::nw.getdata()` function and is licensed under [CC BY-NC 4.0](http://creativecommons.org/licenses/by-nc/4.0/?ref=chooser-v1). Users are free to access and modify the data for non-commercial use with proper citation. By downloading the data, the user agrees to have read the below terms of use prior to use. 
...
## Attempts Data
`NW.attemps` contains nest-level parameters. These fields include data such as species, coordinates, nest substrate, presence fo predator guards, etc as well as summary data for the nest attempt such as success/failure, clutch size, first lay date, fledge date, etc.

```{r inspect attempts, echo = F, eval= T}
library(kableExtra)
library(DT)
#kable(NW.attempts, "html") %>% kable_styling("striped") #%>% scroll_box(width = "100%", height = "300px")
...
`NW.checks` contains information about the status and contents of the the nest during each individual nest check, including number of eggs, you, or dead young of both the host and any brood parasites. 

```{r inspect checks, echo = F, eval= T}
library(kableExtra)
#kable(NW.checks, "html") %>% kable_styling("striped") %>% scroll_box(width = "100%", height = "300px")
