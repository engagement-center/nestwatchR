...
There are two data products available for download via the function [`nestwatchR::nw.getdata()`](https://engagement-center.github.io/nestwatchR/reference/nw.getdata.html). The only argument, `version = `, to this function defines the version of the dataset to be downloaded and defaults to the latest version if left blank. This function will download, unzip, and import two dataframes to the global environment. Depending on your connection speed, this may take a few minutes. 


```{=gfm}
> [!NOTE]
> This function depends on the library [`reticulate`](https://rstudio.github.io/reticulate/) which requires `Python` to be installed on your machine. See the [installation instructions](https://engagement-center.github.io/nestwatchR/index.html#installation-computer) for more information.
```
...
This function uses Python within R to access the Mendeley data archive and download the Nest Watch datasets. Once run, two dataframes will be available in the global environment: `NW.attempts` and `NW.checks`. These dataframes are yet not saved to your machine--you may wish to use [`write.csv()`](https://www.rdocumentation.org/packages/AlphaPart/versions/0.9.8/topics/write.csv) or [saveRDS()](https://rdrr.io/r/base/readRDS.html) to access these data again without the need to re-download the datasets. 
