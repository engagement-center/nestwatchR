...
Our ability to inform conservation efforts
depends on detailed information about the life histories, distributions,
abundances, and population trends of species over time. The goal of the
[NestWatch](https://nestwatch.org), administered by [The Cornell
Lab of Ornithology](https://www.birds.cornell.edu/home/), is to serve as a global, open access repository for data on the rproductive success of birds. NestWatch preserves data collected by members of the global participatory science community
as well as professional researchers. The dataset, and these R tools, are freely available to enable the study of a suite of nesting parameters. NestWatch is designed to
track status and trends in the reproductive biology of birds, including
when nesting occurs, number of eggs laid, how many eggs hatch, and how
many hatchlings survive to fledging. Our database is intended to be used
to study the current condition of breeding bird populations and how they
may be changing over time as a result of climate change, habitat
degradation and loss, expansion of urban areas, and the introduction of
non-native plants and animals. Researchers or members of the public interested in archiving nesting data for current and future use should visit the [NestWatch web site](https://nestwatch.org). Large datasets may be uploaded by contacting the [NestWatch team](mailto:nestwatch@cornell.edu).
...
found in the Nest Watch data paper, [Bailey et al.
...
> ⚠️ This function depends on the library
> [`reticulate`](https://rstudio.github.io/reticulate/) which requires
> `Python` to be installed on your machine. See the [installation
> instructions](https://engagement-center.github.io/nestwatchR/index.html#installation-computer)
...
This function uses Python within R to access the Mendeley data archive
and download the Nest Watch datasets. Once run, two dataframes will be
available in the global environment: `NW.attempts` and `NW.checks`.
These dataframes are yet not saved to your machine--you may wish to use
...
<summary>`r kableExtra::text_spec("**Click for Field Descriptions**", color = "#FF9417")`</summary>
...
dataframes, and having a single merged dataset more easily facilitates
these steps. The two dataframes can be merged using
